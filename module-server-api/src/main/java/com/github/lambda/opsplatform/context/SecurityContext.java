...
public class SecurityContext {
  ...
  public static SessionProtocol of(Principal principal) {
    ...
    if (principal instanceof OAuth2AuthenticationToken) {
      ...
      return SessionProtocol.builder().userId(userId).email(email).roles(roles).authenticated(true)
          .build();
    }

    return SessionProtocol.builder().userId(null).email(null).roles(List.of()).authenticated(false)
        .build();
  }
